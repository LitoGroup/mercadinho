...
export function BottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-20 border-t border-texto/8 bg-white pb-[env(safe-area-inset-bottom)] sm:hidden"
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center gap-1 py-2.5 text-[11px] font-medium ${
                active ? 'text-azul' : 'text-texto/35'
              }`}
            >
              <span className="relative">
                <Icon className="h-6 w-6" strokeWidth={active ? 2.2 : 1.8} />
                {tab.href === '/carrinho' && count > 0 && (
                  <span className="absolute -right-2.5 -top-1.5 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-azul px-1 text-[10px] font-bold text-white">
                    {count}
                  </span>
                )}
              </span>
